#include <iostream>
#include <vector>
#include <iomanip>

class CRC {
private:
    static const uint32_t POLYNOMIAL = 0x11021; // Generatorpolynom (x^16 + x^12 + x^5 + 1) CRC-16-CCITT False
    static std::vector<uint16_t> crcTable; 

public:
    CRC() {
        if (crcTable.empty()) {
            initializeCRCTable();
        }
    }

    static void initializeCRCTable() {
        crcTable.resize(256);
        for (int i = 0; i < 256; ++i) {
            uint16_t crc = i << 8; 
            for (int j = 0; j < 8; ++j) {
                if (crc & 0x8000) {
                    crc = (crc << 1) ^ POLYNOMIAL;
                } else {
                    crc = crc << 1;
                }
            }
            crcTable[i] = crc;
        }
    }

    uint16_t calculateCRC16(const std::vector<unsigned char>& data) {
        uint16_t crc = 0xFFFF; // Initialwert für CRC alles eins weil CRC-16-CCITT False

        for (unsigned char byte : data) {
            uint8_t tableIndex = ((crc >> 8) ^ byte) & 0xFF;
            crc = (crc << 8) ^ crcTable[tableIndex];
        }

        return crc;
    }

    bool verifyDataWithCRC(const std::vector<unsigned char>& dataWithCRC) {
        uint16_t crc = calculateCRC16(dataWithCRC);
        return (crc == 0);
    }
};

std::vector<uint16_t> CRC::crcTable;